...
type EditProfileInfoBoxT = {
  profile: ProfileType
  updateProfile: (profile: ProfileType) => void
  setEditMode: (mode: boolean) => void
}

export const EditProfileInfoBox: FC<EditProfileInfoBoxT> = (props) => {

  const {profile, updateProfile, setEditMode} = props

  const [validateStatus, setValidateStatus] = useState<ValidateStatus>('')
  const [UsernameValidateStatus, UsernameSetValidateStatus] = useState<ValidateStatus>('')
  
  const onFinish = (values: Store) => { 
    updateProfile(values as ProfileType)
    setEditMode(false)
  }
