...
import { Button, Spin, PageHeader, Empty } from 'antd'
...
const DialogE: FC<DialogType> = (props) => {
    ...
    const {dialog, ownerId, isDialogExist, messageText,
        onMessageSend, redirectToProfile, onMessageChange} = props 

    return (
        <div style={{display: 'flex', flexDirection: 'column'}}>
            { !dialog
            ? isDialogExist ? <Spin></Spin> : <Empty></Empty>
            : <div >
                <div style={{height:'8%', position: 'relative', background: '#f0f2f5', width: '100%', 
                borderBottom: '1px solid rgba(0, 0, 0, 0.35)', color: 'rgba(0, 0, 0, 0.65)' }} onClick={() => redirectToProfile(dialog.authors.filter(author => author._id !== ownerId)[0]._id)} >
                    <PageHeader
                        title={`${dialog.authors.filter(author => author._id !== ownerId)[0].firstname} ${dialog.authors.filter(author => author._id !== ownerId)[0].lastname}`}
                        className="site-page-header"
                        avatar={{ src: dialog.authors.filter(author => author._id !== ownerId)[0].image, size:'large'}}
                    />
                </div>
